...
def test_parse_returns_application_with_cidr_entries():
    results = parse(SAMPLE)
    by_application = {r["application"]: [] for r in results}
    for r in results:
        by_application[r["application"]].append(r["cidr"])

    assert "BINSS" in by_application
    assert "217.61.249.0/28" in by_application["BINSS"]
    assert "217.61.249.16/28" in by_application["BINSS"]


def test_parse_handles_dual_stack():
    results = parse(SAMPLE)
    cyter = [r for r in results if r["application"] == "Cyter/Checkov Kommunikation"]
...
def test_parse_expands_comma_listed_ips_to_slash32():
    results = parse(SAMPLE)
    clausberg = sorted(r["cidr"] for r in results if r["application"] == "Clausberg")
    assert clausberg == ["185.91.196.6/32", "185.91.196.7/32", "185.91.196.8/32"]


def test_parse_captures_inline_note():
    results = parse(SAMPLE)
    swe = [r for r in results if r["application"] == "SWE" and r["cidr"] == "217.61.249.192/30"]
    assert len(swe) == 1
...
def test_parse_single_ip_without_prefix_becomes_slash32():
    ...
    assert len(varia) == 1
    assert varia[0]["application"] == "SWE"
    assert "Varia" in varia[0]["notes"]


def test_parse_falcon_extracts_both_v4_and_v6_networks():
    results = parse(SAMPLE)
    falcon = sorted(r["cidr"] for r in results if r["application"] == "Falcon")
...
def test_parse_does_not_double_count_host_ip_lines():
    ...
    results = parse(SAMPLE)
    binss_v4 = [r["cidr"] for r in results if r["application"] == "BINSS"]
